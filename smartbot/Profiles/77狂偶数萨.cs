      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 图腾数量
            int numberTotems = board.MinionFriend.Count(card => card.Race == Card.CRace.TOTEM)+board.MinionFriend.Count(x => x.Template.Id == Card.Cards.TSC_069);
            Bot.Log("图腾数"+numberTotems);
 #endregion
 #region 石雕凿刀 REV_917 
            if(board.HasCardInHand(Card.Cards.REV_917)
            &&!board.HasCardInHand(Card.Cards.ULD_413)
            &&board.WeaponFriend == null
            ){
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.REV_917, new Modifier(-350));
                Bot.Log("石雕凿刀 -350");
            }
            if(board.WeaponFriend != null 
            && board.WeaponFriend.Template.Id == Card.Cards.REV_917
            ){
                p.GlobalWeaponsAttackModifier = -50;
                Bot.Log("石雕凿刀a");
            }
#endregion
 #region 掩息海星 TSC_926 
...
#endregion
 #region 深渊魔物 OG_028
            if(board.HasCardInHand(Card.Cards.OG_028)
            ){
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.OG_028, new Modifier(-50));
            Bot.Log("深渊魔物后置出");
            }
#endregion
 #region 驻锚图腾 TSC_922 
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(-98)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(600));
            Bot.Log("驻锚图腾 -98");
            }
            if(board.HasCardInHand(Card.Cards.TSC_922)
            &&board.WeaponFriend != null 
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(-999)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(600));
            Bot.Log("驻锚图腾 -999");
            }
#endregion
...
#endregion
 #region 怪盗图腾 EVIL Totem ID：ULD_276 
...
#endregion
#region 分裂战斧 Splitting Axe  ID：ULD_413
            //费用等于4，手上有分裂斧且场上无图腾，降低分裂战斧优先级，优先摇图腾
                ...
                {
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.ULD_413, new Modifier(350)); //分裂战斧 Splitting Axe  ID：ULD_413
 ...
 #region 图腾之力 
        ...
        if ((board.HasCardInHand(Card.Cards.ULD_171)||(board.HasCardInHand(Card.Cards.EX1_244)))
            &&numberTotems >= 3
        ...
        if ((board.HasCardInHand(Card.Cards.ULD_171)||(board.HasCardInHand(Card.Cards.EX1_244)))
         ...
         if (
            ...
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.ULD_171, new Modifier(-50));
            Bot.Log("图腾潮涌慢点出");
            }
