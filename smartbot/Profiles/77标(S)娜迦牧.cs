...
#endregion
 #region 侍女 TSC_212
       if(board.HasCardInHand(Card.Cards.TSC_212)
      &&numberCemeterySpells<4
      ){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_212, new Modifier(999));
        Bot.Log("侍女 999");
      }
       if(board.HasCardInHand(Card.Cards.TSC_212)
      ){
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_212, new Modifier(-100));
      }
#endregion
 #region 光照元素 CORE_UNG_034 毒蛇假发 TSC_215 
       if(board.HasCardOnBoard(Card.Cards.CORE_UNG_034)
      &&board.MaxMana <3
      ){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_215, new Modifier(-99, Card.Cards.CORE_UNG_034));
        Bot.Log("小于3费,毒蛇假发保光照");
      }
#endregion
