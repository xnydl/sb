      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 光明法术数量 绝望祷言 BAR_309 炽燃圣光 REV_249 纳鲁的赐福 AV_330 纳鲁碎片 SW_441 罪罚（等级1） BAR_314
            int numberLightSpells=board.Hand.Count(x => x.Template.Id == Card.Cards.BAR_309)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.REV_249)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.BAR_314)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.BAR_314t)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.BAR_314t2)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.SW_441)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.REV_252)//净场 REV_252
            +board.Hand.Count(x => x.Template.Id == Card.Cards.REV_252t)//净场 REV_252t
            +board.Hand.Count(x => x.Template.Id == Card.Cards.GVG_008)//圣光炸弹 GVG_008
            +board.Hand.Count(x => x.Template.Id == Card.Cards.AV_330);
            Bot.Log("光明法术数量"+numberLightSpells);

            // 暗影法术
             int numberShadowSpells=board.Hand.Count(x => x.Template.Id == Card.Cards.CS3_028)//暗中生长 CS3_028
            +board.Hand.Count(x => x.Template.Id == Card.Cards.EX1_197)//暗言术：毁 EX1_197
            +board.Hand.Count(x => x.Template.Id == Card.Cards.DED_512);//不朽护符 DED_512
            Bot.Log("暗影法术"+numberShadowSpells);
            // 亡语随从数量 灵魂向导 AV_328
            int numberDeadLanguageFollowers=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_328)
            +board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_310)//光沐元素 BAR_310
            +board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.CS3_024)//泰兰·弗丁 Taelan Fordring ID：CS3_024 
            +board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_325);//不死信徒 Undying Disciple ID：AV_325 
            Bot.Log("亡语随从数量"+numberDeadLanguageFollowers);
            

 #endregion
#region 遍历随从
            // 坟场2费卡数量
            // int cemetery2FeecaQuantity=0;
            // for (int x = 0; x < board.FriendGraveyard.Count; x++)
            //     {
            //         if(board.FriendGraveyard.Count[x].CurrentCost==2){
            //           cemetery2FeecaQuantity+=1;  
            //         }
            //     }
            // Bot.Log("坟场2费卡数量"+cemetery2FeecaQuantity);
#endregion
#region 不送的怪
...
#endregion
 #region 法术反制 Counterspell ID：CORE_EX1_287 
       if(board.HasCardInHand(Card.Cards.CORE_EX1_287)){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_287, new Modifier(-99)); 
        Bot.Log("法术反制 -99");
      }
#endregion
 #region 湍流旋涡 Whirlpool ID：TSC_209 
       if(board.HasCardInHand(Card.Cards.TSC_209)
       &&skeletalMage>5){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_209, new Modifier(-99)); 
        Bot.Log("湍流旋涡 -99");
      }
#endregion
 #region 炽燃圣光 REV_249 
       if(board.HasCardInHand(Card.Cards.REV_249)){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_249, new Modifier(150)); 
        Bot.Log("炽燃圣光 150");
      }
#endregion
 #region 雷象坐骑 Elekk Mount ID：SW_443 
       if(board.HasCardInHand(Card.Cards.SW_443)){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_443, new Modifier(-40)); 
        Bot.Log("雷象坐骑 -40");
      }
#endregion
 #region 纳鲁的赐福 AV_330
       if(board.HasCardInHand(Card.Cards.AV_330)
       &&board.HeroFriend.CurrentHealth<=36){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_330, new Modifier(-99)); 
        Bot.Log("纳鲁的赐福 -99");
      }
#endregion
 #region 纳鲁碎片 SW_441
       if(board.HasCardInHand(Card.Cards.SW_441)
       &&board.FriendDeckCount==0){
       	p.TradeModifiers.AddOrUpdate(Card.Cards.SW_441, new Modifier(999)); 
        Bot.Log("纳鲁碎片不交易");
      }
#endregion
 #region 不朽护符 DED_512
       if(board.HasCardInHand(Card.Cards.DED_512)
       &&board.FriendDeckCount==0){
       	p.TradeModifiers.AddOrUpdate(Card.Cards.DED_512, new Modifier(999)); 
        Bot.Log("不朽护符不交易");
      }
       if(board.HasCardInHand(Card.Cards.DED_512)
       &&numberDeadLanguageFollowers>=2){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DED_512, new Modifier(-99)); 
        Bot.Log("不朽护符-99");
      }
#endregion
 #region 暗中生长 CS3_028
       if(board.HasCardInHand(Card.Cards.CS3_028)){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS3_028, new Modifier(-150)); 
        Bot.Log("暗中生长 -150");
      }
#endregion
 #region 雷纳索尔王子 REV_018
       if(board.HasCardInHand(Card.Cards.REV_018)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_018, new Modifier(-99)); 
        Bot.Log("雷纳索尔王子 -99");
      }
#endregion
 #region 模糊的陌生人 REV_945
       if(board.HasCardInHand(Card.Cards.REV_945)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_945, new Modifier(-99)); 
        Bot.Log("模糊的陌生人 -99");
      }
#endregion
 #region 圣徒泽瑞拉 Xyrella, the Sanctified ID：SW_433t3 
       if(board.HasCardInHand(Card.Cards.SW_433t3)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_433t3, new Modifier(-999)); 
        Bot.Log("圣徒泽瑞拉 -999");
      }
#endregion
 #region 赎罪教堂 REV_290
       if(board.HasCardInHand(Card.Cards.REV_290)){
        p.LocationsModifiers.AddOrUpdate(Card.Cards.REV_290, new Modifier(-150)); 
        Bot.Log("赎罪教堂 -150");
      }
#endregion
 #region 泽瑞拉 BAR_735  
       if(board.HasCardInHand(Card.Cards.BAR_735)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_735, new Modifier(150)); 
        Bot.Log("泽瑞拉 150");
      }
#endregion
 #region 纳亚克·海克森 Najak Hexxen ID：AV_331 
       if(board.HasCardInHand(Card.Cards.AV_331)
       &&board.MinionEnemy.Count ==0){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_331, new Modifier(350)); 
        Bot.Log("纳亚克·海克森 350");
      }
#endregion
 #region 净化的碎片 Purified Shard ID：SW_433t3a 
       if(board.HasCardInHand(Card.Cards.SW_433t3a)){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_433t3a, new Modifier(-9999)); 
        Bot.Log("秒杀!");
      }
#endregion
 #region 光腹虚空幼龙 ONY_026
            if(board.HasCardInHand(Card.Cards.ONY_026)
            &&(numberShadowSpells==0||numberLightSpells==0)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.ONY_026, new Modifier(350)); 
            Bot.Log("光腹虚空幼龙 350");
            }
            if(board.HasCardInHand(Card.Cards.ONY_026)
            &&board.MinionEnemy.Count == 0  
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.ONY_026, new Modifier(150)); 
            Bot.Log("光腹虚空幼龙 150");
            }
#endregion
 #region 深铁穴居人  AV_137  
