...
#region 攻击模式和自定义 
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int numberCemeterySpells=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Type == Card.CType.SPELL);
            // 大于五费的法术 大法师的符文 Rune of the Archmage ID：AV_283 
            int moreThanFiveFeeSpell=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_283)
            +board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.REV_840) //死神之躯 Deathborne ID：REV_840 
            +board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.ONY_029); //龙火护符 ONY_029
            Bot.Log("坟场大于五费的法术数量"+moreThanFiveFeeSpell);
            int moreThanFiveFeeSpellinhand=board.Hand.Count(x => x.Template.Id == Card.Cards.AV_283)//大法师的符文 AV_283
            +board.Hand.Count(x => x.Template.Id == Card.Cards.REV_840)
            +board.Hand.Count(x => x.Template.Id == Card.Cards.ONY_029);//龙火护符 ONY_029
            Bot.Log("手上大于五费的法术数量"+moreThanFiveFeeSpellinhand);
    //    我方坟场骷髅数量
            int numberSkeletonsInOurCemetery=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.REV_845); 
             Bot.Log("坟场骷髅数量"+numberSkeletonsInOurCemetery);
            // 场上2血随从数量
            int number2BloodFollowersOnField=board.MinionEnemy.Count(minion => minion.CurrentHealth <= 2)+board.MinionFriend.Count(minion => minion.CurrentHealth <= 2);
 ...
 #region 送的怪
             // 不稳定的骷髅 Volatile Skeleton ID：REV_845
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.REV_845, new Modifier(-5)); //修饰老巨鳍之口 Gigafin's Maw ID：TSC_962t 
#endregion
//  #region 指挥官西瓦拉 Commander Sivara ID：TSC_087 
//             if(board.Hand(card=>card.Powered==true && card=>CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_087)
//             ){
//             p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_087, new Modifier(-150)); 
//             Bot.Log("指挥官西瓦拉"+(-150));
//             }
// #endregion
 #region 野蛮的女巫 TSC_020
            if(board.HasCardInHand(Card.Cards.TSC_020)
            &&moreThanFiveFeeSpellinhand>0
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_020, new Modifier(-150)); 
            Bot.Log("野蛮的女巫"+(-150));
            }
#endregion
 #region 冰风暴（等级2） Flurry (Rank 2) ID：BAR_305t 
            if(board.HasCardInHand(Card.Cards.BAR_305t)
            &&enemyAttack>=6
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_305t, new Modifier(55-enemyAttack)); 
            Bot.Log("冰风暴（等级2）"+(55-enemyAttack));
            }
#endregion
 #region 冰风暴（等级3） Flurry (Rank 3) ID：BAR_305t2 
            if(board.HasCardInHand(Card.Cards.BAR_305t2)
            &&enemyAttack>=6
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_305t2, new Modifier(55-enemyAttack)); 
            Bot.Log("冰风暴（等级3）"+(55-enemyAttack));
            }
#endregion
 #region 奥术智慧 CORE_CS2_023
            if(board.HasCardInHand(Card.Cards.CORE_CS2_023)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_CS2_023, new Modifier(-99)); 
            Bot.Log("奥术智慧 -99");
            }
#endregion
 #region 死神之躯 Deathborne ID：REV_840 
            if(board.HasCardInHand(Card.Cards.REV_840)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_840, new Modifier(-99*number2BloodFollowersOnField)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_840, new Modifier(-50));
            Bot.Log("死神之躯"+(-99*number2BloodFollowersOnField));
            }
#endregion
 #region 脱罪力证 Solid Alibi ID：REV_504 
            if(board.HasCardInHand(Card.Cards.REV_504)
            &&enemyAttack>board.HeroFriend.CurrentHealth
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_504, new Modifier(-99)); 
            Bot.Log("脱罪力证 -99");
            }
#endregion
 #region 冰冻之触 Frozen Touch ID：REV_601 
            if(board.HasCardInHand(Card.Cards.REV_601)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_601, new Modifier(150)); 
            Bot.Log("冰冻之触 150");
            }
#endregion
 #region 冰冷案例 Cold Case ID：REV_505
            if(board.HasCardInHand(Card.Cards.REV_505)
            &&board.MinionFriend.Count <6
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_505, new Modifier(-999)); 
            Bot.Log("冰冷案例 -999");
            }else{
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_505, new Modifier(150)); 
            }
#endregion
 #region 夜隐者圣所 REV_602
            if(board.HasCardInHand(Card.Cards.REV_602)
            &&friendCount<=6
            ){
            p.LocationsModifiers.AddOrUpdate(Card.Cards.REV_602, new Modifier(-999)); 
            Bot.Log("夜隐者圣所 -999");
            }
#endregion
 #region 灰贤鹦鹉 Grey Sage Parrot ID：DED_515 
            if(board.HasCardInHand(Card.Cards.DED_515)
            &&moreThanFiveFeeSpell==0
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_515, new Modifier(350)); 
            Bot.Log("灰贤鹦鹉 350");
            }else{
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_515, new Modifier(-99)); 
            }
#endregion
 #region 海中向导芬利爵士 Sir Finley, Sea Guide ID：TSC_908 
            if(board.HasCardInHand(Card.Cards.TSC_908)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_908, new Modifier(150)); 
            Bot.Log("海中向导芬利爵士 150");
            }
#endregion
 #region 纳兹夏尔女士 Lady Naz'jar ID：TID_709  
            if(board.HasCardInHand(Card.Cards.TID_709)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TID_709, new Modifier(350)); 
            Bot.Log("纳兹夏尔女士 350");
            }
#endregion
 #region 鲁莽的学徒 Reckless Apprentice ID：BAR_544 
            if(board.HasCardInHand(Card.Cards.BAR_544)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_544, new Modifier(350)); 
            Bot.Log("鲁莽的学徒 350");
            }
#endregion
 #region 指挥官西瓦拉 Commander Sivara ID：TSC_087 
            if(board.HasCardInHand(Card.Cards.TSC_087)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_087, new Modifier(150)); 
            Bot.Log("指挥官西瓦拉 150");
            }
#endregion
 #region 瓦尔登·晨拥 Varden Dawngrasp ID：BAR_748 
            if(board.HasCardInHand(Card.Cards.BAR_748)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_748, new Modifier(350)); 
            Bot.Log("瓦尔登·晨拥 350");
            }
#endregion
 #region 多系施法者 Multicaster ID：DED_524  
            if(board.HasCardInHand(Card.Cards.DED_524)
            &&board.Hand.Count>3
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_524, new Modifier(150)); 
            Bot.Log("多系施法者 150");
            }
#endregion
...
#endregion
#region 火眼莫德雷斯 BAR_547
        if(board.HasCardInHand(Card.Cards.BAR_547)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_547, new Modifier(350)); 
                Bot.Log("火眼莫德雷斯 350");
            }
#endregion
#region 天定之灾克尔苏加德 Kel'Thuzad, the Inevitable ID：REV_514 
        if(board.HasCardInHand(Card.Cards.REV_514)
        &&(numberSkeletonsInOurCemetery*2+myAttack)<board.HeroEnemy.CurrentHealth){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_514, new Modifier(350)); 
            Bot.Log("天定之灾克尔苏加德 350");
        }else{
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_514, new Modifier(-999)); 
        }
#endregion
 #region 野火 BAR_546
            if(board.HasCardInHand(Card.Cards.BAR_546)
            ){
            p.CastSpellsModifiers.AddOrUpdate( Card.Cards.BAR_546, new Modifier(-9999)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.BAR_546, new Modifier(999));
            Bot.Log("野火 -9999");
            }
#endregion
