...
#endregion
 #region 通灵领主德拉卡 REV_940
       if(board.HasCardInHand(Card.Cards.REV_940)
      ){
       p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_940, new Modifier(-100));
      }
#endregion
 #region 罪碑坟场 REV_750
       if(board.HasCardInHand(Card.Cards.REV_750)
      ){
       p.LocationsModifiers.AddOrUpdate(Card.Cards.REV_750, -99);
        Bot.Log("罪碑坟场 -99");
      }
#endregion
 #region 黑水弯刀 DED_004
       if(board.HasCardInHand(Card.Cards.DED_004)
      ){
        p.TradeModifiers.AddOrUpdate(Card.Cards.DED_004, -999);
        Bot.Log("黑水弯刀交易");
      }
#endregion
 #region 暗影之门 Door of Shadows ID：REV_938 
       if(board.HasCardInHand(Card.Cards.REV_938)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_938, new Modifier(999)); 
        Bot.Log("暗影之门 999");
      }
#endregion
 #region 暗影之门 Door of Shadows REV_938t  
       if(board.HasCardInHand(Card.Cards.REV_938t)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_938t, new Modifier(-20)); 
        Bot.Log("暗影之门+ -20");
      }
#endregion
 #region 偷师学艺 BAR_323
       if(board.HasCardInHand(Card.Cards.BAR_323)
      ){
       p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_323, new Modifier(-20)); 
        Bot.Log("偷师学艺 -20");
      }
#endregion
 #region 掩息海星 TSC_926 
