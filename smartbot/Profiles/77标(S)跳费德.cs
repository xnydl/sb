...
#region 直伤卡牌 标准模式
        ...
        private static readonly Dictionary<Card.Cards, int> _spellDamagesTable = new Dictionary<Card.Cards, int>
        {
            ...
            //德鲁伊
            {Card.Cards.AT_037a, 2},//缠人根须 Grasping Roots ID：AT_037a 
            ...
            {Card.Cards.REV_307, 2},//自然死亡 Natural Causes ID：REV_307 
        };
        //直伤随从卡牌（必须可以打脸）
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            Bot.Log("不稳定的骷髅数量"+skeletalMage);
             ...
             // 占领冷齿矿洞 AV_295
            int flag1=board.MinionFriend.Count(x => x.Template.Id == Card.Cards.AV_295)+board.Hand.Count(x => x.Template.Id == Card.Cards.AV_295)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_295);
            // 普瑞斯托女士 SW_078
            int flag2=board.MinionFriend.Count(x => x.Template.Id == Card.Cards.SW_078)+board.Hand.Count(x => x.Template.Id == Card.Cards.SW_078)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SW_078);
            // 野性之心古夫 Wildheart Guff ID：AV_205 
            int flag3=board.MinionFriend.Count(x => x.Template.Id == Card.Cards.AV_205)+board.Hand.Count(x => x.Template.Id == Card.Cards.AV_205)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_205);
            int dangqianfeiyong = board.ManaAvailable;
            // 纳迦巨人 TSC_829 森林之王伊弗斯 AV_142t
            int flag4=board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_829)+
            board.Hand.Count(x => x.Template.Id == Card.Cards.AV_142t);
 #endregion
#region 不送的怪
...
#region 艾露恩神谕者 SW_419 
    if(board.HasCardInHand(Card.Cards.SW_419))
    {
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_419, new Modifier(350)); 
    }
     if(board.HasCardInHand(Card.Cards.SW_419)
    &&board.Hand.Exists(x=>x.CurrentCost<=2 && x.Template.Id==Card.Cards.TSC_829)//纳迦巨人 TSC_829
    )
   {
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_419, new Modifier(-99)); 
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_829, new Modifier(-99)); 
    p.PlayOrderModifiers.AddOrUpdate(Card.Cards.SW_419, new Modifier(999)); 
    Bot.Log("艾露恩神谕者+纳迦巨人");
    }
     if(board.HasCardInHand(Card.Cards.SW_419)
    &&board.HasCardInHand(Card.Cards.AV_142t) //森林之王伊弗斯 AV_142t
    &&board.ManaAvailable>=15
    )
   {
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_419, new Modifier(-99)); 
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_142t, new Modifier(-99)); 
    p.PlayOrderModifiers.AddOrUpdate(Card.Cards.SW_419, new Modifier(999)); 
    Bot.Log("艾露恩神谕者+森林之王伊弗斯");
    }
#endregion
#region 暗礁德鲁伊 DED_001
 ...
 #region 剑圣萨穆罗 BAR_078
      ...
      &&board.MinionEnemy.Count !=1
       	...
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_078, new Modifier(650)); 
        Bot.Log("剑圣萨穆罗 650");
      }
#endregion
 #region 并蒂树苗 One, Two, Trees! ID：AT_037b 
       if(board.HasCardInHand(Card.Cards.AT_037b)
      &&friendCount<6
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_037b, new Modifier(-20)); 
        Bot.Log("并蒂树苗 -20");
      }
#endregion
 #region 快速生长 Rampant Growth ID：EX1_164a 
       if(board.HasCardInHand(Card.Cards.EX1_164a)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_164a, new Modifier(-20)); 
        Bot.Log("快速生长 -20");
      }
#endregion
...
#endregion
 #region 摄取养分 Enrich ID：EX1_164b 
       if(board.HasCardInHand(Card.Cards.EX1_164b)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_164b, new Modifier(-20)); 
        Bot.Log("摄取养分 -20");
      }
#endregion
 #region 万灵之召 Convoke the Spirits ID：REV_365 
       if(board.HasCardInHand(Card.Cards.REV_365)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_365, new Modifier(-99)); 
        Bot.Log("万灵之召 -99");
      }
#endregion
 #region 自然死亡 Natural Causes ID：REV_307 
       if(board.HasCardInHand(Card.Cards.REV_307)
      &&friendCount<7
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_307, new Modifier(-20)); 
        Bot.Log("自然死亡 -20");
      }
#endregion
 #region 缠人根须 Grasping Roots ID：AT_037a 
       if(board.HasCardInHand(Card.Cards.AT_037a)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_037a, new Modifier(350)); 
      }
#endregion
 #region 号令虎鲸 Order the Orca ID：TSC_650a 
       if(board.HasCardInHand(Card.Cards.TSC_650a)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_650a, new Modifier(-99)); 
      }
#endregion
 #region 罪恶谋划 REV_336 
       if(board.HasCardInHand(Card.Cards.REV_336)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_336, new Modifier(350)); 
        Bot.Log("罪恶谋划 350");
      }
#endregion
 #region 罪恶谋划 REV_336t4 
       if(board.HasCardInHand(Card.Cards.REV_336t4)
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_336t4, new Modifier(-99)); 
        Bot.Log("罪恶谋划 -99");
      }
#endregion
 #region 水栖形态 Aquatic Form ID：TSC_654 
       if(board.HasCardInHand(Card.Cards.TSC_654)
      &&board.ManaAvailable<3
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_654, new Modifier(350)); 
        Bot.Log("水栖形态 350");
      }
#endregion
 #region 安插证据 Planted Evidence ID：REV_313
       if(board.HasCardInHand(Card.Cards.REV_313)
       &&board.ManaAvailable>=3
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_313, new Modifier(-99)); 
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_313, new Modifier(999)); 
        Bot.Log("安插证据 -99");
      }else{
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_313, new Modifier(350));
      }
#endregion
...
#region 更多补给 More Supplies ID：AV_295b 
      ...
      p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_295b, new Modifier(-999));
      Bot.Log("更多补给  -999");
      }
#endregion
...
#endregion
 #region 自然使徒 TID_002 
        if(board.HasCardInHand(Card.Cards.TID_002)
        &&friendCount<3
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TID_002, new Modifier(350));  
            Bot.Log("自然使徒 350");
        }
#endregion
 #region 灌木巨龙托匹奥 Topior the Shrubbagazzor ID：REV_314 
        if(board.HasCardInHand(Card.Cards.REV_314)
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_314, new Modifier(-350));  
            Bot.Log("灌木巨龙托匹奥 -350");
        }
#endregion
 #region 划水好友 Flipper Friends ID：TSC_650 
        if(board.HasCardInHand(Card.Cards.TSC_650 )
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_650 , new Modifier(-20));  
            Bot.Log("划水好友-20");
        }
#endregion
 #region 应急木工 Jerry Rig Carpenter ID：DED_003 
        if(board.HasCardInHand(Card.Cards.DED_003)
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_003, new Modifier(-99));  
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.DED_003, new Modifier(999)); 
            Bot.Log("应急木工 -99");
        }
#endregion
#region 应急木工 DED_003 
...
#endregion
#region 激活 Innervate ID：CORE_EX1_169 
        // 如果一费有恶魔法证则不用
        if(board.HasCardInHand(Card.Cards.CORE_EX1_169)){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_169, new Modifier(55));
        }
#endregion

#region 贪食的吞噬者 Insatiable Devourer ID：REV_017  
       if(board.HasCardInHand(Card.Cards.REV_017)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_017, new Modifier(250)); 
        Bot.Log("贪食的吞噬者 250");
      }
#endregion
#region 贪食的吞噬者 Insatiable Devourer ID：REV_017t 
       if(board.HasCardInHand(Card.Cards.REV_017t)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_017t, new Modifier(150)); 
        Bot.Log("贪食的吞噬者+ 150");
      }
#endregion
...
#region 德纳修斯大帝 Sire Denathrius ID：REV_906t 
        if(board.HasCardInHand(Card.Cards.REV_906t)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(350)); 
            }
        if(board.HasCardInHand(Card.Cards.REV_906)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906, new Modifier(350)); 
            }
#endregion
#region 范达尔·鹿盔 Fandral Staghelm ID：OG_044 
        if(board.HasCardInHand(Card.Cards.OG_044)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.OG_044, new Modifier(350)); 
            }
#endregion
#region 布莱恩·铜须 Brann Bronzebeard ID：CORE_LOE_077 
            ...
            &&board.ManaAvailable >= 9
       	    ...
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_LOE_077, new Modifier(-999)); 
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(-998)); 
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(-997)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.CORE_LOE_077, new Modifier(9999));
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(9998));
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(9997));
            Bot.Log("打combe 铜须-逐罪者凯尔萨斯-德纳修斯大帝");
            }    
#endregion
#region 逐罪者凯尔萨斯 Kael'thas Sinstrider ID：REV_021 
    if(board.HasCardInHand(Card.Cards.REV_021)){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(350)); 
        }
    //    if(board.HasCardInHand(Card.Cards.REV_021)
    //    &&board.ManaAvailable >= 9
    //    &&board.Hand.Exists(card => card.CurrentCost<=3)){
    //    	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(-99)); 
    //     p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(1000));
    //     Bot.Log("逐罪者凯尔萨斯 -99");
    //   }else{
    //    	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_021, new Modifier(130)); 
    //   }
#endregion
