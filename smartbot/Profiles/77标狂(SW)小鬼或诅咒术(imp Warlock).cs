...
// 德：DRUID 猎：HUNTER 法：MAGE 骑：PALADIN 牧：PRIEST 贼：ROGUE 萨：SHAMAN 术：WARLOCK 战：WARRIOR 瞎：DEMONHUNTER
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 拥有的诅咒牌数量
            int numberCurseCardsOwn=board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_955)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_956)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_924);
            // 下回合可以造成的诅咒伤害
            int truenumberCurses=(numberCurses*numberCurseCardsOwn)+1;
            // Bot.Log("拥有的诅咒牌数量"+numberCurseCardsOwn);
...
#region 不送的怪
 ...
 #region 送的怪
          ...
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.REV_373t, new Modifier(-20)); //具象暗影 Shadow Manifestation ID：REV_373t 
#endregion
...
#region 深渊诅咒 Abyssal Curse ID：TSC_955t 
 ...
 #region 深渊波流 TSC_924
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_924, new Modifier(130)); 
            Bot.Log("深渊波流 130");
            }    
#endregion
 #region 牺牲魔典 Grimoire of Sacrifice ID：BAR_910 
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_910, new Modifier(350)); 
            Bot.Log("牺牲魔典 350");
            }else{
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_910, new Modifier(130)); 
            } 
#endregion
 #region 拖入深渊 TSC_956
 ...
 #region 深渊诅咒 Abyssal Curse ID：TSC_955t 
            ...
            &&numberCurseCardsOwn>0
                ...
                // 希拉柯丝教徒 TSC_955
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_955, new Modifier(-150));
                // 扎库尔 TSC_959
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_959, new Modifier(-150));
                // 拖入深渊 TSC_956
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_956, new Modifier(-150)); 
                // 深渊波流 TSC_924
            // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_924, new Modifier(-99)); 
            Bot.Log("咒杀!");
            }    
#endregion
#region 塔姆辛·罗姆 BAR_918
            if(board.HasCardInHand(Card.Cards.BAR_918)
            &&board.HasCardInHand(Card.Cards.SW_090)//纳斯雷兹姆之触 SW_090
            &&board.ManaAvailable >= 4
            &&bloodVolumeEquals2>=2
            ){
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_918, new Modifier(-99)); 
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_090, new Modifier(-99)); 
            Bot.Log("血量小于等于2的敌方怪为2,提高解场combes");
            }
            if(board.HasCardInHand(Card.Cards.BAR_918)
            &&board.HasCardInHand(Card.Cards.TSC_956)//拖入深渊 TSC_956
            &&bloodVolumeEquals4>=1
            &&board.ManaAvailable >= 6
            ){
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_918, new Modifier(-99)); 
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_956, new Modifier(-99)); 
            Bot.Log("血量大于等于4的敌方怪大于0,提高解场combes");
            }
            if(board.HasCardInHand(Card.Cards.BAR_918)
            &&board.HasCardInHand(Card.Cards.TSC_924)//深渊波流 TSC_924
            &&bloodVolumeEquals4>=2
            &&board.ManaAvailable >= 9
            ){
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_918, new Modifier(-99)); 
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_924, new Modifier(-99)); 
            Bot.Log("血量大于等于4的敌方怪大于0,提高解场combes 情况2");
            }
            if(board.HasCardInHand(Card.Cards.BAR_918)
            ){
 ...
 #region 布莱恩·铜须 Brann Bronzebeard ID：CORE_LOE_077 
            if(board.HasCardInHand(Card.Cards.CORE_LOE_077)
            &&board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.REV_906t)==0 
            ){
...
#region 希拉柯丝教徒 TSC_955
 ...
 #region 暗脉女勋爵 Lady Darkvein ID：REV_373 
            ...
            &&board.MinionFriend.Count >5
       	    ...
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_373, new Modifier(350)); 
            Bot.Log("暗脉女勋爵 350");
            }
            if(board.HasCardInHand(Card.Cards.REV_373)//拖入深渊 TSC_956 深渊波流 TSC_924 
            &&board.MinionFriend.Count <=5
            &&(board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_956)>0
            ||board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_924)>0
            )
            ){
       	    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_373, new Modifier(-250)); 
            Bot.Log("暗脉女勋爵 -250");
            }
#endregion
...
#region 阴暗的酒保 Shady Bartender ID：SW_086 
           ...
           p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_086, new Modifier(550));
            ...
            p.TradeModifiers.AddOrUpdate(Card.Cards.SW_086, new Modifier(350));
...
#region 调皮的小鬼 Mischievous Imp ID：REV_244t 
       ...
       &&board.MinionFriend.Count <7){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_244t, new Modifier(-99)); 
        Bot.Log("调皮的小鬼+ -99");
      }
#endregion
...
#region 邪恶图书馆 Vile Library ID：REV_371 
       ...
       if(board.HasCardOnBoard(Card.Cards.REV_371)
        ...
        p.LocationsModifiers.AddOrUpdate(Card.Cards.REV_371, new Modifier(-99,Card.Cards.CORE_CS2_065));
      }
#endregion
#region 灾祸降临 REV_245
// 如果场上随从大于3,且手牌数+场上随从数小于10,提高抽牌法术优先级 灾祸降临 REV_245
       ...
       &&board.FriendDeckCount >=numberKids
       ){
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_245, new Modifier(-50*numberKids)); 
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_245, new Modifier(-50));
        Bot.Log("灾祸降临"+(-50*numberKids));
      }else{
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_245, new Modifier(150)); 
      }
#endregion
#region 恶魔法阵 CORE_GIL_191 
       ...
       &&board.MinionFriend.Count <=3
       	...
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_GIL_191, new Modifier(-99)); 
        Bot.Log("恶魔法阵-99");
      }
       if(board.HasCardInHand(Card.Cards.CORE_GIL_191)
       &&board.MinionFriend.Count >=6
...
#region 小鬼集群（等级1） Imp Swarm (Rank 1) ID：BAR_914
       if(board.HasCardInHand(Card.Cards.BAR_914)
       	...
       	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_914, new Modifier(130)); 
        Bot.Log("小鬼集群（等级1）"+(130));
      }
#endregion
#region 邪恶船运 Wicked Shipment ID：DED_504 
       if(board.HasCardInHand(Card.Cards.DED_504)
       &&(board.MinionFriend.Count<2||board.MaxMana <=3 )
       ){
...
#region 生命分流 Life Tap   HERO_07bp
        ...
        // &&board.ManaAvailable ==2)
        // { 
        // p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_07bp, new Modifier(-30));//生命分流 Life Tap ID：HERO_07bp
        // Bot.Log("抽一口");
        // // p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_316hp, new Modifier(1000));
        // }
#endregion
...
#region 德纳修斯大帝 Sire Denathrius ID：REV_906t 
        if(board.HasCardInHand(Card.Cards.REV_906t)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(999)); 
            }
        if(board.HasCardInHand(Card.Cards.REV_906)){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906, new Modifier(999)); 
            }
        //  if(board.HasCardOnBoard(Card.Cards.CORE_LOE_077)
        //     &&board.HasCardOnBoard(Card.Cards.REV_021)//逐罪者凯尔萨斯 Kael'thas Sinstrider ID：REV_021
        //     &&board.HasCardInHand(Card.Cards.REV_906t)//德纳修斯大帝 Sire Denathrius ID：REV_906t
        //     &&board.ManaAvailable >= 9
        //     ){
       	//     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(-9999)); 
        //     p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(9999));
        //     Bot.Log("打combe 铜须-逐罪者凯尔萨斯-德纳修斯大帝2");
        //     }    
        //  if(board.HasCardInHand(Card.Cards.CORE_LOE_077)
        //     &&board.HasCardOnBoard(Card.Cards.REV_021)//逐罪者凯尔萨斯 Kael'thas Sinstrider ID：REV_021
        //     &&board.ManaAvailable >= 9
        ...
        //     Bot.Log("打combe 铜须-逐罪者凯尔萨斯-德纳修斯大帝3");
        //     }    
#endregion
#region 大帝伤害
        ...
        var flameImpId3 = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.REV_906t);//德纳修斯大帝 Sire Denathrius ID：REV_906t
            if(SireDenathriusCard != null)
            {
                ...
                &&board.HasCardInHand(Card.Cards.REV_906t)//德纳修斯大帝 Sire Denathrius ID：REV_906t
                &&board.ManaAvailable >= 9
                &&((sireDenaDamages*2+myAttack)>=enemyMinionHealth+board.HeroEnemy.CurrentHealth||board.Hand.Count==3||enemyAttack>board.HeroFriend.CurrentHealth)
                ){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_LOE_077, new Modifier(-9999)); 
                ...
                &&board.HasCardInHand(Card.Cards.REV_906t)//德纳修斯大帝 Sire Denathrius ID：REV_906t
                &&((sireDenaDamages*2)>=enemyMinionHealth+board.HeroEnemy.CurrentHealth||board.Hand.Count==3||enemyAttack>board.HeroFriend.CurrentHealth)
                ){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_906t, new Modifier(-9999)); 
