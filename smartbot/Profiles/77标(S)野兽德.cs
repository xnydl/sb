...
#endregion
#region 迷宫向导 Maze Guide ID：REV_308 
    // 如果自己场上有活泼的松鼠,送掉它
    if(board.HasCardInHand(Card.Cards.REV_308)
    &&board.MinionFriend.Count <6)
    {
    p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_308, new Modifier(999));
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_308, new Modifier(-150)); 
    Bot.Log("迷宫向导,-150");
    }
#endregion
#region 森林之王伊弗斯 Ivus, the Forest Lord ID：AV_142t 
    // 如果自己场上有活泼的松鼠,送掉它
    if(board.HasCardInHand(Card.Cards.AV_142t)
    &&board.ManaAvailable >= 5)
    {
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_142t, new Modifier(-150)); 
    Bot.Log("森林之王伊弗斯,-150");
    }
#endregion
#region 农夫 SW_319 
...
#region 施肥 Composting     SW_437
    // 施肥起手一定别留，施肥一般4费之后才打出，这牌只能起到锦上添花的作用
    ...
    { 
      ...
      p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_437, new Modifier(150));
    }
#endregion
#region 暗礁德鲁伊 DED_001
 ...
 #region 万灵之召 Convoke the Spirits ID：REV_365 
       if(board.HasCardInHand(Card.Cards.REV_365)
        ...
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.REV_365, new Modifier(-999)); 
        Bot.Log("万灵之召 -999");
      }
#endregion
 ...
 #region 缠人根须 Grasping Roots ID：AT_037a 
    ...
    //   ){
    //     p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_037a, new Modifier(999)); 
    //   }
#endregion
 #region 罪恶谋划 REV_336 
...
//  如果手里没有使徒，一定要在1费的时候水栖探底找，这样2费抽到，打出自然法术，3费可以下
// 还有手里完全没有1费随从也可以探底，除了这两种情况，建议3费或者4费才探底
      ...
      &&board.ManaAvailable<3
       ...
       if(board.HasCardInHand(Card.Cards.TSC_654)
      &&board.MaxMana==1
      &&!board.HasCardInHand(Card.Cards.TID_002)//自然使徒 TID_002
      ){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_654, new Modifier(-99)); 
        Bot.Log("手里没有使徒，一定要在1费的时候水栖探底找");
      }
#endregion
...
#region 更多资源 More Resources ID：AV_295a 
 ...
 #region 灌木巨龙托匹奥 Topior the Shrubbagazzor ID：REV_314 
        if(board.HasCardInHand(Card.Cards.REV_314)
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_314, new Modifier(-150));  
            Bot.Log("灌木巨龙托匹奥 -150");
        }
#endregion
 #region 划水好友 Flipper Friends ID：TSC_650 
        if(board.HasCardInHand(Card.Cards.TSC_650 )
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_650 , new Modifier(-99));  
            Bot.Log("划水好友-99");
        }
#endregion
