...
#region 空军指挥官艾克曼 Wing Commander Ichman ID：AV_336 
       ...
       &&friendCount<=6){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_336, new Modifier(-999)); 
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_336, new Modifier(999));
        Bot.Log("空军指挥官艾克曼 -150");
      }
#endregion
#region 自然使徒 Herald of Nature ID：TID_002 
      if(board.HasCardInHand(Card.Cards.TID_002)){
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TID_002, new Modifier(130));
        Bot.Log("自然使徒 130");
      }
#endregion
...
#region 鲁莽的学徒 Reckless Apprentice ID：BAR_544 
      ...
      &&board.MinionEnemy.Count <=1
        ...
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_544, new Modifier(150)); 
        Bot.Log("鲁莽的学徒 150");
      }
#endregion
...
#endregion
#region 丹巴达尔桥 Dun Baldar Bridge ID：AV_344 
         if(board.HasCardInHand(Card.Cards.AV_344)){
          p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_344, new Modifier(999));
          Bot.Log("丹巴达尔桥 优先出");
      }
#endregion
#region 野性之心 Heart of the Wild ID：AV_292 
...
#region 攻击优先 卡牌威胁
            ...
            if (board.MinionEnemy.Any(minion => minion.Template.Id == Card.Cards.TOT_308))
             {
                p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.TOT_308, new Modifier(200));
            }//洞穴梦游者 Cavern Dreamer ID：TOT_308
            ...
            }//机械鲨鱼 Mecha-Shark ID：TSC_054 
            if (board.MinionEnemy.Any(minion => minion.Template.Id == Card.Cards.BAR_840))
             {
                p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.BAR_840, new Modifier(200));
            }//旋风争斗者 Whirling Combatant ID：BAR_840 
