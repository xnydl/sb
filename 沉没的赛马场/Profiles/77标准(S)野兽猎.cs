      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int flag1=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Race  == Card.CRace.PET);
            Bot.Log("坟场野兽"+flag1);
            int flag2=board.MinionFriend.Count;
            Bot.Log("场上随从数"+flag2);
            // 沉没的刃豹 Sunken Saber ID：TSC_945t 鼠王 The Rat King ID：SW_323 海卓拉顿 Hydralodon ID：TSC_950 山岭野熊 Mountain Bear ID：AV_337 暴龙王克鲁什 King Krush ID：CORE_EX1_543 
            int flag3=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_945t)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SW_323)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_950)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.AV_337)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.CORE_EX1_543);
            Bot.Log("坟场高质量随从数"+flag3);
...
#endregion
#region 送的怪
        if(board.HasCardOnBoard(Card.Cards.SW_323)){
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.SW_323, new Modifier(-10)); //鼠王 The Rat King ID：SW_323 
        }
        if(board.HasCardOnBoard(Card.Cards.TSC_945)){
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_945, new Modifier(-10)); //艾萨拉的刃豹 Azsharan Saber ID：TSC_945

        }
        if(board.HasCardOnBoard(Card.Cards.TSC_945t)){
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_945t, new Modifier(-10)); //沉没的刃豹 Sunken Saber ID：TSC_945t
...
#endregion
#region 空军指挥官艾克曼 Wing Commander Ichman ID：AV_336 
       if(board.HasCardInHand(Card.Cards.AV_336)
       &&flag2<=6){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_336, new Modifier(-150)); 
        Bot.Log("空军指挥官艾克曼 -150");
      }
#endregion
#region 如果坟场没有相应的野兽 不用复活宠物 复活宠物 Revive Pet ID：AV_333 
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_333, new Modifier(-550)); 
        if(flag1==0||flag2==7){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_333, new Modifier(999)); 
        }
        if(flag3>0
        &&board.HasCardInHand(Card.Cards.AV_333)){
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_333, new Modifier(-99)); 
      Bot.Log("用复活宠物");
        }
#endregion
#region Card.Cards.HERO_05bp
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(-550)); 
#endregion
#region 选种饲养员 Selective Breeder ID：CS3_015 
            if(board.HasCardInHand(Card.Cards.CS3_015)
            ){
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.CS3_015, new Modifier(888));
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CS3_015, new Modifier(-150)); 
            Bot.Log("选种饲养员 优先级提高");
            }
#endregion
#region 艾萨拉的刃豹 Azsharan Saber ID：TSC_945
            if(board.HasCardInHand(Card.Cards.TSC_945)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_945, new Modifier(-99)); 
            Bot.Log("艾萨拉的刃豹 -99");
            }
#endregion
#region 追踪术 Tracking ID：CORE_DS1_184 
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_DS1_184, new Modifier(350)); 
            Bot.Log("追踪术 350");
            }
#endregion
#region 如果坟场有艾萨拉的刃豹 Azsharan Saber ID：TSC_945 ,提高宠物收集者 Pet Collector ID：ONY_009 
            if(board.HasCardInHand(Card.Cards.ONY_009)
            // &&board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.TSC_945)<2
            ){
