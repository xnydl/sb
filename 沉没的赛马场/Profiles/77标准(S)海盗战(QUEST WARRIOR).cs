      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 海盗数量
            int haidaonum=board.Hand.Count(card => card.Race == Card.CRace.PIRATE)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_069);//深海融合怪 Amalgam of the Deep ID：TSC_069  ;
            Bot.Log("手上海盗数量"+haidaonum);
            int changshanghaidaonum=board.MinionFriend.Count(card => card.Race == Card.CRace.PIRATE)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_069);//深海融合怪 Amalgam of the Deep ID：TSC_069  ;;
