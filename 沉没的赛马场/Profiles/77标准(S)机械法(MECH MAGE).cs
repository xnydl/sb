            ...
            }          
       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int aomiCount = board.Secret.Count;
             int flag1 = board.Hand.Count(card => card.Race == Card.CRace.MECHANICAL)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_069);//深海融合怪 Amalgam of the Deep ID：TSC_069  
            Bot.Log("手上机器数量"+flag1);
             int flag2 = board.MinionFriend.Count(card => card.Race == Card.CRace.MECHANICAL); 
            Bot.Log("场上机器数量"+flag2);
            int flag3=board.Hand.Count(card => card.Type == Card.CType.MINION);
            Bot.Log("手上随从数量"+flag3);
            // 通电机器人 BOT_907
...
#endregion
#region 不送的怪
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_054, new Modifier(150)); //修饰机械鲨鱼 TSC_054
#endregion
#region 海床传送口 TSC_055  
      if(board.HasCardInHand(Card.Cards.TSC_055)
      &&!board.HasCardInHand(Card.Cards.GIL_548)
      ){
      p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TSC_055, new Modifier(-99*flag1));
       p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_055, new Modifier(9999)); 
      Bot.Log("海床传送口"+-99*flag1);
      }
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(55));
#endregion
#region 怨灵之书 GIL_548  
      if(board.HasCardInHand(Card.Cards.GIL_548)){
      p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GIL_548, new Modifier(-9999));
      Bot.Log("怨灵之书 -9999");
      }
#endregion
#region 龙火护符 Drakefire Amulet ID：ONY_029 
      if(board.Hand.Count(x=>x.CurrentCost==5 && x.Template.Id==Card.Cards.ONY_029)>=1){
      p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ONY_029, new Modifier(-999));
      Bot.Log("龙火护符 -999");
      }
#endregion
...
#endregion
#region 战栗的女巫 Shivering Sorceress ID：AV_114 
      if(board.HasCardInHand(Card.Cards.AV_114)
      &&(board.HasCardInHand(Card.Cards.TSC_055)||board.HasCardInHand(Card.Cards.CORE_CS2_029))//火球术 Fireball ID：CORE_CS2_029 
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_114, new Modifier(-99));
      Bot.Log("战栗的女巫 -99");
      }else{
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_114, new Modifier(150));
      }
#endregion
#region 机械鲨鱼 TSC_054
      ...
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_054, new Modifier(150));
      }
#endregion
...
#region 通电机器人 BOT_907
      ...
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BOT_907, new Modifier(-100*flag1));
      }
#endregion
...
#endregion
#region 艾萨拉的清道夫 Azsharan Sweeper ID：TSC_776 
      if(board.HasCardInHand(Card.Cards.TSC_776)){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_776, new Modifier(-200));
      p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_776, new Modifier(999)); 
      Bot.Log("艾萨拉的清道夫 -200");
      }
#endregion
#region 沉没的清道夫 Sunken Sweeper ID：TSC_776t
      if(board.HasCardInHand(Card.Cards.TSC_776t)){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_776t, new Modifier(-250));
