...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int friendCount = board.MinionFriend.Count;
             int numberFishMen=board.Hand.Count(card => card.Race == Card.CRace.MURLOC)+board.Hand.Count(x => x.Template.Id == Card.Cards.TSC_069);//深海融合怪 Amalgam of the Deep ID：TSC_069  ;
            Bot.Log("手上鱼人数量"+numberFishMen);
             int numberFishMenFriend=board.MinionFriend.Count(card => card.Race == Card.CRace.MURLOC)+board.MinionFriend.Count(x => x.Template.Id == Card.Cards.TSC_069);
...
#region 提高一些怪的出牌优先级
          ...
          p.PlayOrderModifiers.AddOrUpdate(Card.Cards.SW_115, new Modifier(999));//伯尔纳·锤喙 SW_115
#endregion
#region Card.Cards.GAME_005 小丑鱼 TID_004 
           if(board.HasCardInHand(Card.Cards.GAME_005)
            &&board.HasCardInHand(Card.Cards.TID_004)
            &&board.MaxMana == 1
            ){
...
#region 鳄鱼人掠夺者 Gorloc Ravager ID：TSC_034 
            ...
            // ){
            // p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_034, new Modifier(-150)); 
            // Bot.Log("鳄鱼人掠夺者 -150");
            // }
#endregion
#region 寒光先知 Coldlight Seer ID：CORE_EX1_103 
            ...
            Bot.Log("寒光先知"+-20*(numberFishMenFriend));
            }else{
...
#region  鱼勇可贾 BAR_041
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_041, new Modifier(-20*(friendCount))); 
            Bot.Log("鱼勇可贾"+-20*(numberFishMenFriend));
            }else{
...
#region 火焰术士弗洛格尔 BAR_860 
            if(board.HasCardInHand(Card.Cards.BAR_860)
              ...
              p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_860, new Modifier(150));
              Bot.Log("火焰术士弗洛格尔150");
            }
#endregion
#region 鱼人领军 Murloc Warleader ID：CORE_EX1_507 
            if(board.HasCardInHand(Card.Cards.CORE_EX1_507)
              ...
              p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_507, new Modifier(150));
              Bot.Log("鱼人领军 150");
            }
#endregion
#region 甜水鱼人斥候 Lushwater Scout BAR_063
            ...
            // &&board.MaxMana <=2 
            // ){
            //   p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_063, new Modifier(-99));
            //   Bot.Log("甜水鱼人斥候 -99");
            // }
#endregion
#region Card.Cards.HERO_02bp
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.HERO_02bp, new Modifier(-550)); 
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_258p2 , new Modifier(-550));//流水祈咒 Water Invocation ID：AV_258p2  
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_258pt3, new Modifier(-550));//火焰祈咒 Fire Invocation ID：AV_258pt3  
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_258pt, new Modifier(-550));//大地祈咒 Earth Invocation ID：AV_258pt  
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_258pt4, new Modifier(-550));//闪电祈咒 Lightning Invocation ID：AV_258pt4 
#endregion
#region 攻击优先 卡牌威胁
