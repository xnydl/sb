...
#region 奥术爆裂 Arcane Burst ID：AV_200p2 
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_200p2, new Modifier(999)); 
        p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.AV_200p2, new Modifier(-30));
#endregion
#region 野火 BAR_546
         if(board.HasCardInHand(Card.Cards.BAR_546)
      ){
     p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_546, new Modifier(-999));
      Bot.Log("野火"+-999);
      }
#endregion
#region 大法师的符文 AV_283
         if(board.HasCardInHand(Card.Cards.AV_283)
      ){
     p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_283, new Modifier(-99));
      Bot.Log("大法师的符文"+-99);
      }
#endregion
...
#endregion
#region 鲁莽的学徒 BAR_544
         if(board.HasCardInHand(Card.Cards.BAR_544)
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_544, new Modifier(130));
      Bot.Log("鲁莽的学徒"+130);
      }
#endregion
#region 巴琳达·斯通赫尔斯 AV_284
         if(board.HasCardInHand(Card.Cards.AV_284)
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_284, new Modifier(-999));
      Bot.Log("巴琳达·斯通赫尔斯"+-999);
      }
#endregion
#region 深水唤醒师 DED_516
         if(board.HasCardInHand(Card.Cards.DED_516)
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_516, new Modifier(-150));
      Bot.Log("深水唤醒师"+-150);
      }
#endregion
#region 巡游向导 SCH_312
         if(board.HasCardInHand(Card.Cards.SCH_312)
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(130));
      Bot.Log("巡游向导"+130);
      }
#endregion
#region 笨拙的信使 SW_109
         if(board.HasCardInHand(Card.Cards.SW_109)
         &&board.HasCardInHand(Card.Cards.AV_283)//大法师的符文 AV_283
      ){
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_109, new Modifier(-999));
      Bot.Log("笨拙的信使"+-999);
      }else{
      p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_109, new Modifier(130));
