...
#region 攻击模式和自定义 
      ...
      {
int denum=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Type == Card.CType.MINION);
...
#region  随从优先级
    ...
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_223, new Modifier(-999));//99范达尔·雷矛 Vanndar Stormpike ID：AV_223 
...
#endregion
#region 范达尔·雷矛 AV_223
          if(board.HasCardInHand(Card.Cards.AV_223)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AV_223, new Modifier(-150)); 
             Bot.Log("德雷克塔尔 -150");
            }
             if(board.HasCardInHand(Card.Cards.AV_223)
             &&board.ManaAvailable<=2
...
#endregion
#region 恩佐斯，深渊之神 N'Zoth, God of the Deep ID：DMF_002 
          if(board.HasCardInHand(Card.Cards.DMF_002)&&board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Type == Card.CType.MINION)>=2
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_002, new Modifier(-15*denum)); 
             Bot.Log("恩佐斯，深渊之神 "+(-15*denum));
            }
            
#endregion
#region 哥利亚，斯尼德的杰作 Goliath, Sneed's Masterpiece ID：DED_525 
          if(board.HasCardInHand(Card.Cards.DED_525)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DED_525, new Modifier(130)); 
             Bot.Log("哥利亚，斯尼德的杰作 130");
            }
            
#endregion
#region 克苏恩，破碎之劫 C'Thun, the Shattered ID：DMF_254 
          if(board.HasCardInHand(Card.Cards.DMF_254)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_254, new Modifier(-999)); 
             Bot.Log("克苏恩，破碎之劫 -999");
            }
            
#endregion
#region 戈霍恩之血 Blood of G'huun ID：DMF_053
          if(board.HasCardInHand(Card.Cards.DMF_053)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_053, new Modifier(-150)); 
             Bot.Log("戈霍恩之血 -150");
            }
            
#endregion
#region 不朽护符 Amulet of Undying ID：DED_512 
          if(board.HasCardInHand(Card.Cards.DED_512)&&board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Type == Card.CType.MINION)>=2
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DED_512, new Modifier(-5*denum));    
                     Bot.Log("不朽护符"+(-5*denum));
            }
            
#endregion
#region 复苏 Renew ID：BT_252 
          if(board.HasCardInHand(Card.Cards.BT_252)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_252, new Modifier(130));    
                     Bot.Log("复苏"+(-130));
            }
            
#endregion
#region 纳鲁的赐福 Gift of the Naaru ID：AV_330 
          if(board.HasCardInHand(Card.Cards.AV_330)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_330, new Modifier(-20));    
                     Bot.Log("纳鲁的赐福"+(-20));
            }
            
#endregion
#region 灵魂之镜 Soul Mirror ID：BT_198
          if(board.HasCardInHand(Card.Cards.BT_198)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_198, new Modifier(130));    
                     Bot.Log("灵魂之镜 130");
            }
            
#endregion
#region 纳兹曼尼织血者 Nazmani Bloodweaver ID：DMF_120
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_120,new Modifier(-999,Card.Cards.SCH_302));//流光之赐 Gift of Luminance ID：SCH_302
