...
#region 增加随从威胁值
   p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.BT_155, new Modifier(200));//废料场巨像 Scrapyard Colossus     BT_155 
    ...
    p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.BAR_918, new Modifier(250));//塔姆辛·罗姆 Tamsin Roame     BAR_918 
...
// ){
//   p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_514, new Modifier(-20));
//   Bot.Log("亡者复生 -40");
// }
...
#region 教导主任加丁 SCH_126 
            if(board.HasCardInHand(Card.Cards.SCH_126)
            &&board.ManaAvailable <= 3
            &&(board.HasCardInHand(Card.Cards.SCH_145)||board.HasCardInHand(Card.Cards.LOEA10_3))
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_145, new Modifier(999)); 
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOEA10_3, new Modifier(999));  
            Bot.Log("有教导主任加丁不用0费怪四费前");
            }
            if(board.HasCardInHand(Card.Cards.SCH_126)
            &&(board.HasCardInHand(Card.Cards.SCH_145)||board.HasCardInHand(Card.Cards.LOEA10_3))
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_126, new Modifier(-99)); 
            Bot.Log("教导主任加丁 -99");
            }
            if(board.HasCardInHand(Card.Cards.SCH_126)
...
#region 阴暗的酒保 Shady Bartender ID：SW_086 
           ...
           p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_086, new Modifier(130));
            Bot.Log("阴暗的酒保 130");
            }
#endregion
#region 骨网之卵 SCH_147 
           ...
           p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_147, new Modifier(130));
            Bot.Log("骨网之卵 130");
            }
#endregion
#region 狗头人图书管理员 LOOT_014 
            if(board.HasCardInHand(Card.Cards.LOOT_014)
            ){
           p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_014, new Modifier(-99));
            Bot.Log("狗头人图书管理员 -99");
            }
#endregion
#region 霜狼将领 AV_134
            if(board.HasCardInHand(Card.Cards.AV_134)
            ){
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.AV_134, new Modifier(-50)); 
            Bot.Log("霜狼将领慢点出");
            }
#endregion
...
#endregion
#region 小精灵 VAN_CS2_231 
            if(board.HasCardInHand(Card.Cards.VAN_CS2_231)
            ){
             p.CastMinionsModifiers.AddOrUpdate(Card.Cards.VAN_CS2_231, new Modifier(-99));
            Bot.Log("小精灵 -99");
            }
            if(board.HasCardInHand(Card.Cards.CS2_231)
            ){
             p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CS2_231, new Modifier(-99));
            Bot.Log("小精灵2 -99");
            }
#endregion
...
#endregion
#region 加拉克苏斯之拳 Fist of Jaraxxus ID：AT_022 
            if(board.HasCardInHand(Card.Cards.AT_022)
            ){
           p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_022, new Modifier(130));
            Bot.Log("加拉克苏斯之拳 130");
            }
#endregion
...
#endregion
// #region 末日仪式 CORE_CS3_002
//             if(board.HasCardInHand(Card.Cards.CORE_CS3_002)
//             &&board.MinionFriend.Count >=5
//             ){
//            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_CS3_002, new Modifier(-99));
//             Bot.Log("末日仪式 -99");
//             }else{
//            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_CS3_002, new Modifier(999));
//             }
// #endregion
...
#region 被亵渎的墓园 Desecrated Graveyard ID：AV_657 
            ...
            if(board.HasCardInHand(Card.Cards.AV_657)
            ){
           p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_657, new Modifier(-999));
            Bot.Log("被亵渎的墓园 -999");
            }
#endregion
...
#region 邪恶船运 Wicked Shipment ID：DED_504
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DED_504, new Modifier(999));
            Bot.Log("邪恶船运 999");
            }
#endregion
