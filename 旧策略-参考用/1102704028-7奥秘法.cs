...
namespace SmartBotProfiles
{
    ...
    public class WildSecretMage : Profile
    {
        ...
        public ProfileParameters GetParameters(Board board)
        {
            ...
            //卡牌优先度调整
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_003, new Modifier(-15));//虚空之风传送门 Netherwind Portal  ID：BT_003
            ...
            // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_003, new Modifier(-20));//提高虚空之风传送门 Netherwind Portal ID：BT_003优先度
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_107, new Modifier(-15));//提高非公平游戏 Rigged Faire Game ID：DMF_107 优先度
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ULD_726, new Modifier(-20));//降低远古谜团优先度
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS2_023, new Modifier(-10));//提高奥术智慧优先度
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_005, new Modifier(280));//降低变猪优先度
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ULD_239, new Modifier(-5));//降低火焰结界优先度
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.UNG_020, new Modifier(-15));//降低秘法学家优先度
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_428, new Modifier(-20));//博学者普克尔特 Lorekeeper Polkelt  ID：SCH_428优先度
            ...
            int LibraryResidueSecret = 9 - (board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).IsSecret) + board.Hand.Count(card => card.Template.IsSecret) + board.Secret.Count);//牌库剩余奥秘
            ...
            if (board.ManaAvailable >= 6 && board.Hand.Count <= 6 && board.HasCardInHand(Card.Cards.DMF_109) || LibraryResidueSecret<=6)
            {
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.DMF_109, new Modifier(50));
            }//如果当前剩余奥秘小于等于6，手牌数量小于等于5并且手牌中有暗月先知赛格 Sayge, Seer of Darkmoon ID：DMF_109，提高暗月先知赛格 Sayge, Seer of Darkmoon ID：DMF_109的优先级	



            //随从调整
             if (board.ManaAvailable >= 4
              && (board.HasCardInHand(Card.Cards.DMF_106))//隐秘咒术师 Occult Conjurer ID：DMF_106
              && board.Secret.Count > 0)
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_106, new Modifier(100));
            }
            else
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_106, new Modifier(180));
            }//增加隐秘咒术师 Occult Conjurer ID：DMF_106的优先级
                ...
                && board.MinionFriend.Any(minion => minion.Template.Id == Card.Cards.FP1_004))
            {
             ...
             if (LibraryResidueSecret > 0
                && board.HasCardInHand(Card.Cards.FP1_004))//疯狂的科学家 Mad Scientist ID：FP1_004
            {
               p.CastMinionsModifiers.AddOrUpdate(Card.Cards.FP1_004, new Modifier(-40));//疯狂的科学家 Mad Scientist ID：FP1_004
            }//牌库有奥秘，提高科学家的优先度

            if (LibraryResidueSecret > 0
                && board.HasCardInHand(Card.Cards.UNG_020))//秘法学家 Arcanologist ID：UNG_020
            {
               p.CastMinionsModifiers.AddOrUpdate(Card.Cards.UNG_020, new Modifier(-20));//秘法学家 Arcanologist ID：UNG_020
            }//牌库有奥秘，提高秘法学家 Arcanologist ID：UNG_020的优先度
            ...
            //对阵慢速或者没怪，提高非公平优先级
            if(board.HasCardInHand(Card.Cards.DMF_107)//非公平游戏 Rigged Faire Game ID：DMF_107 
            ...
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.LOOT_101, new Modifier(-40));//非公平游戏 Rigged Faire Game ID：DMF_107 
            }else{
                 p.CastSpellsModifiers.AddOrUpdate(Card.Cards.LOOT_101, new Modifier(-15));//非公平游戏 Rigged Faire Game ID：DMF_107 
            }

            //对阵慢速或者没怪，提高变形药水 Potion of Polymorph ID：CFM_620优先级
            if(board.HasCardInHand(Card.Cards.CFM_620)//变形药水 Potion of Polymorph ID：CFM_620
            && board.EnemyClass == Card.CClass.MAGE
            || board.EnemyClass == Card.CClass.WARLOCK
            || board.EnemyClass == Card.CClass.DRUID
            || board.EnemyClass == Card.CClass.PRIEST
            ...
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CFM_620, new Modifier(-60));//变形药水 Potion of Polymorph ID：CFM_620
            }else{
                 p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CFM_620, new Modifier(-15));//变形药水 Potion of Polymorph ID：CFM_620
            }
              ...
              //如果牌库剩余数量小于已用奥秘数量,不使用暗月
            if(board.HasCardInHand(Card.Cards.DMF_109)//暗月先知赛格 Sayge, Seer of Darkmoon ID：DMF_109
            && board.FriendDeckCount<LibraryResidueSecret
             )
                 
            {
               
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.DMF_109, new Modifier(350));//暗月先知赛格 Sayge, Seer of Darkmoon ID：DMF_109
            } 
