...
namespace SmartBotProfiles
{
    ...
    public class WildDemonHunter : Profile
    {
		...
		//version 2.4.1 for 19.0HS
            ...
            // {
            //     p.GlobalAggroModifier = (int)(a * 0.625 + 110);
            // }
            // else
            // {
            //     p.GlobalAggroModifier = (int)(a * 0.625 + 110);
        ...
        //     else
         ...
         //    if (board.HeroFriend.CurrentHealth <= 10)      //血小于等于10             
         //    {
          //       p.GlobalAggroModifier = (int)(a * 0.625 + 50.5);            //则切换为控场模式
          //   }
...
//todo坟场
                var deadNum =  board.FriendGraveyard.Count;
                // Bot.Log("死亡随从数量为:" + deadNum);
                // bool deadNum;
                // if(
...
//todo一费相关
            ...
            && board.HasCardInHand(Card.Cards.LOOT_014)//狗头人图书管理员 Kobold Librarian  ID：LOOT_014
            && board.HasCardInHand(Card.Cards.SCH_312)//巡游向导 Tour Guide  ID：SCH_312
            || board.HasCardInHand(Card.Cards.KAR_089)//玛克扎尔的小鬼 Malchezaar's Imp  ID：KAR_089
            )
            {
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_014, new Modifier(-40));//狗头人图书管理员 Kobold Librarian  ID：LOOT_014
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(500));//巡游向导 Tour Guide  ID：SCH_312
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.KAR_089, new Modifier(500));//玛克扎尔的小鬼 Malchezaar's Imp  ID：KAR_089
...
//todo 小丑
                if(board.HasCardOnBoard(Card.Cards.DMF_163t)//狂欢小丑 Carnival Clown ID：DMF_163t 
                && board.HasCardInHand(Card.Cards.DMF_236)//古神在上 Oh My Yogg! ID：DMF_236 
                || board.HasCardInHand(Card.Cards.GIL_203)//责难 Rebuke ID：GIL_203 
                )
                {
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_236, new Modifier(-60));//古神在上 Oh My Yogg! ID：DMF_236 
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GIL_203, new Modifier(-60));//责难 Rebuke ID：GIL_203 
                }
//-------------------------------------------------------------------------
//todo黑眼术策略
//todo 小鬼
                // //如果费用大于3 场上有马尔扎哈小鬼 提高灵界优先级
                //  if (board.ManaAvailable >= 3
                // && board.HasCardOnBoard(Card.Cards.KAR_089)//玛克扎尔的小鬼 Malchezaar's Imp  ID：KAR_089
                // && board.HasCardInHand(Card.Cards.BOT_568)//莫瑞甘的灵界 The Soularium  ID：BOT_568
                // )
                // {
                //   p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BOT_568, new Modifier(-40));
                // }
...
//todo风怒机器人相关
                ...
                //强化机器人 Enhance-o Mechano ID：GVG_107
                // if (board.HasCardInHand(Card.Cards.GVG_107)
                // && board.HasCardOnBoard(Card.Cards.SCH_140)
                // || board.HasCardOnBoard(Card.Cards.EX1_620)
                // || board.HeroFriend.CurrentHealth <=13
                // || board.HeroEnemy.CurrentHealth <= myAttack
                // )
                // {
                //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GVG_107, new Modifier(-200)); //强化机器人 Enhance-o Mechano  ID：GVG_107
                // }
                // else
                // {
                //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GVG_107, new Modifier(500)); //强化机器人 Enhance-o Mechano  ID：GVG_107
                // }
                 if (board.HasCardOnBoard(Card.Cards.GVG_107)//强化机器人 Enhance-o Mechano ID：GVG_107
                    ...
                    || board.Hand.Count(minion => minion.Template.Id == Card.Cards.EX1_316) ==2//力量的代价 Power Overwhelming  ID：EX1_316
                    && board.MinionFriend.Count >= 1
                    )
                {
                    
                    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GVG_107, new Modifier(-800)); //强化机器人 Enhance-o Mechano  ID：GVG_107
                }
                else
                {
...
//todo晶化师相关
            //     //总血量不少于15不下晶化师
            // if (board.HeroFriend.CurrentHealth >= 15 //自己英雄的健康
...
//todo2费相关
            ...
            //     p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BOT_222, new Modifier(500));//灵魂炸弹 Spirit Bomb  ID：BOT_222
            ...
            //        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BT_307, new Modifier(-100));//黑眼 Darkglare  ID：BT_307
            //         p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DRG_206, new Modifier(-10));//火焰之雨 Rain of Fire  ID：DRG_206
            //     }
...
//todo玛维影歌            
		...
		// //没事不交硬币(SB喜欢第一个交硬币，写这个只是为了以防万一
		// 	if (board.ManaAvailable <= 2)
		// 	{
		// 	//场上或者手上没有黑眼或者秃鹫
		// 		if (board.HasCardOnBoard(Card.Cards.ULD_167)
		// 		|| board.HasCardOnBoard(Card.Cards.BT_307))
		// 		{
		// 			p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TB_011, new Modifier(50));   //硬币
		// 			p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(50));   //硬币
		// 		}
		// 		else
		// 		{
		// 			p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TB_011, new Modifier(999));   //硬币
		// 			p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(999));   //硬币
		// 		}
...
//todo铁钩掠夺者
            ...
            // else
            // {
            //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_018, new Modifier(-20));//铁钩掠夺者 Hooked Reaver  ID：LOOT_018
            // }
            //血量不少于15不下铁钩（不算护甲）
...
//todo黑眼相关
                ...
                )
            {
                ...
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.YOP_033, new Modifier(-1000));//赛车回火 Backfire ID：YOP_033
            }
            ...
            // else
            // {
            //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BT_307, new Modifier(500));//黑眼
            // }
            //  //手上有幸运币，3费可以拍黑眼
            //     if (board.ManaAvailable == 2
            //         && board.HasCardInHand(Card.Cards.GAME_005)
            //         || board.HasCardInHand(Card.Cards.BT_309)//坎雷萨德·埃伯洛克 Kanrethad Ebonlocke  ID：BT_309
            //         && board.HasCardInHand(Card.Cards.BT_307)
                ...
                )
            {
                ...
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_514, new Modifier(-5));//亡者复生 Raise Dead  ID：SCH_514
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.YOP_033, new Modifier(-40));//赛车回火 Backfire ID：YOP_033
                //?随从
                ...
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TRL_512, new Modifier(-50));//调皮的噬踝者 Cheaty Anklebiter  ID：TRL_512
                 p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(-100));//巡游向导 Tour Guide  ID：SCH_312
...
//todo亡者复生相关
            //如果坟场有黑眼或者巨人提高亡者复生 Raise Dead  ID：SCH_514
             if (board.HasCardInHand(Card.Cards.SCH_514)//亡者复生 Raise Dead  ID：SCH_514
               ...
               || board.FriendGraveyard.Contains(Card.Cards.LOOT_014) //自己坟场有凯雷塞斯王子 Prince Keleseth  ID：ICC_851 狗头人图书管理员 Kobold Librarian  ID：LOOT_014
                 )
               {
             ...
             //     else
             //   {
            //    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_514, new Modifier(50));//亡者复生 Raise Dead  ID：SCH_514
            //   }
...
//todo 甩笔侏儒相关
            ...
            //对面是牧师,使用甩笔侏儒 Pen Flinger  ID：SCH_248打自己
            // if ( board.HasCardInHand(Card.Cards.SCH_248)//甩笔侏儒 Pen Flinger  ID：SCH_248
            //    && board.HasCardOnBoard(Card.Cards.BT_307) //场上有黑眼黑眼 Darkglare  ID：BT_307
            //    || board.HasCardInHand(Card.Cards.SCH_140)//血肉巨人 Flesh Giant  ID：SCH_140手里有血肉巨人
            //     )
            // {
            //    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_248, new Modifier(-1500, board.HeroFriend.Id));//提高甩笔侏儒打自己脸优先度甩笔侏儒 Pen Flinger  ID：SCH_248
            // }
...
//todo洛欧塞布相关
            //如果下一轮可以斩杀对面提高洛欧塞布 Loatheb  ID：FP1_030
                ...
                || myAttack >= (board.HeroEnemy.CurrentHealth) /2
					...
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.FP1_030, new Modifier(-80));//洛欧塞布 Loatheb  ID：FP1_030
                }
...
//todo亵渎
            ...
            // )
			// {
            // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ICC_041, new Modifier(200));
			// }else{
            // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ICC_041, new Modifier(50));
            // }
              if (board.ManaAvailable <= 3
                 && board.HasCardInHand(Card.Cards.ICC_041)//亵渎 Defile  ID：ICC_041
...
//todo 技能相关          
        ...
        //   //如果技能为2,用
        //   if (board.Ability.CurrentCost == 2
        //   && board.ManaAvailable != 2
        //   )
        //       {
        //       p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(-100));//巡游向导 Tour Guide  ID：SCH_312
        //       p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS2_056_H1, new Modifier(-99, board.HeroEnemy.Id));
        //       }
            
        //   //如果技能为0,手上有巡游,降低巡游优先级
...
//todo随从相关
           ...
           //p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_248, new Modifier(200)); //甩笔侏儒 Pen Flinger  ID：SCH_248
            ...
            //p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(-));//巡游向导 Tour Guide  ID：SCH_312
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_163, new Modifier(9999));//狂欢小丑 Carnival Clown ID：DMF_163 
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_163t, new Modifier(-9999));//狂欢小丑 Carnival Clown ID：DMF_163t 
...
//todo法术
            ...
            // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_514, new Modifier(50));//亡者复生 Raise Dead  ID：SCH_514
            //   p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TB_011, new Modifier(80));   //硬币
              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(80));   //硬币
              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_128, new Modifier(-200));   //真菌宝藏 Fungal Fortunes ID：BT_128 
              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_135, new Modifier(-150));   //萤火成群 Glowfly Swarm ID：BT_135
//todo直伤法术优先值		
