...
namespace SmartBotProfiles
{
    ...
    public class WildDemonHunter : Profile
    {
        ...
        //想了想，还是给你留句话吧：敏而好学不耻下问，不懂就问，不丢人。不问自取，那叫做偷，丢人。
        //攻击模式
        ...
        public ProfileParameters GetParameters(Board board)
        {
            ...
            {
                p.GlobalAggroModifier = (int)(a * 0.625 + 999);
            }
            else
            {
                p.GlobalAggroModifier = (int)(a * 0.625 + 999);
...

            {
                ...
                //如果场上有相位追猎者 Phase Stalker  ID：DRG_252提高技能优先值
                if (board.HasCardOnBoard(Card.Cards.DRG_252)//相位追猎者 Phase Stalker  ID：DRG_252

                 )
                {
                    p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(5));
                    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(-20)); //巡游向导 Tour Guide ID：SCH_312

                }


                //一费有向导但是没有相位追猎者 一切照旧 如果手上有相位追猎者 则降低射箭优先级
                if (board.ManaAvailable ==1
                     && board.HasCardInHand(Card.Cards.SCH_312)//巡游向导 Tour Guide  ID：SCH_312
                     &&! board.HasCardInHand(Card.Cards.DRG_252)//相位追猎者 Phase Stalker  ID：DRG_252
                 )
                {
                ...
                else
                {
                    ...
                    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_312, new Modifier(-10)); //巡游向导 Tour Guide ID：SCH_312
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(500, board.HeroEnemy.Id));


                }

                //当自己手上没有武器 且自己头上有奥秘 手上耐久度为1
                if ((board.WeaponFriend != null && board.WeaponFriend.Template.Id == Card.Cards.EX1_536)//鹰角弓 Eaglehorn Bow  ID：EX1_536
                    && board.WeaponFriend.CurrentDurability == 1
                    &&! board.HasCardInHand(Card.Cards.EX1_536)//鹰角弓 Eaglehorn Bow  ID：EX1_536
                    || board.HasCardOnBoard(Card.Cards.DRG_252)//相位追猎者 Phase Stalker  ID：DRG_252
                    && board.Secret.Count==0

                    )
                {
                    p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.EX1_536, new Modifier(300));//鹰角弓 Eaglehorn Bow  ID：EX1_536
                }
                ...
                {
                    p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.EX1_536, new Modifier(-60));//鹰角弓 Eaglehorn Bow  ID：EX1_536
                    p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.EX1_536, new Modifier(30));//鹰角弓 Eaglehorn Bow  ID：EX1_536
                   
                }

                //冰冻陷阱在手上可以下
                if (board.ManaAvailable >=4
                    && board.HasCardInHand(Card.Cards.EX1_611)//冰冻陷阱 Freezing Trap  ID：EX1_611


                    )
                {
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_611, new Modifier(-40));//冰冻陷阱 Freezing Trap  ID：EX1_611
                }
                else
                 {
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_611, new Modifier(150));//冰冻陷阱 Freezing Trap  ID：EX1_611
                }
                //费用大于4，提高相位追猎者 Phase Stalker  ID：DRG_252优先级
                if (board.ManaAvailable >=4
                    && board.HasCardInHand(Card.Cards.DRG_252)//相位追猎者 Phase Stalker  ID：DRG_252

                    )
                {
                   p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DRG_252, new Modifier(-40));//相位追猎者 Phase Stalker  ID：DRG_252
                   p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(10, board.HeroEnemy.Id));
                }
                
                //场上有相位追猎者 Phase Stalker  ID：DRG_252优先级 提高射箭优先级
                if (board.HasCardOnBoard(Card.Cards.DRG_252)//相位追猎者 Phase Stalker  ID：DRG_252
                   
                    )
                {
                    p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(-20, board.HeroEnemy.Id));
                }
            ...
            //直伤法术优先值		
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_600, new Modifier(40));//恶魔伙伴 Demon Companion  ID：SCH_600
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DRG_255, new Modifier(-60));//病毒增援 Toxic Reinforcements  ID：DRG_255
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_213, new Modifier(-100));//拾荒者的智慧 Scavenger's Ingenuity  ID：BT_213
           // p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_611, new Modifier(10));//冰冻陷阱 Freezing Trap  ID：EX1_611
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_609, new Modifier(-30));//优胜劣汰 Survival of the Fittest  ID：SCH_609

            //提高猎人的技能生命分流优先级
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_05bp, new Modifier(5, board.HeroEnemy.Id));

            //随从优先值


            //随从优先值
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_617t, new Modifier(-10));//魔鼠宝宝 Marsuul Cub  ID：SCH_617t
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DRG_256, new Modifier(-60));//灭龙弩炮 Dragonbane  ID：DRG_256
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GVG_075, new Modifier(-200));//船载火炮 Ship's Cannon  ID：GVG_075
            //不送的怪
