...
namespace SmartBotProfiles
{
    ...
    public class WildDemonHunter : Profile
    {
        ...
        private static readonly Dictionary<Card.Cards, int> _spellDamagesTable = new Dictionary<Card.Cards, int>
        {
            ...
            {Card.Cards.CS2_075, 3},//影袭 Sinister Strike  ID：CS2_075
        ...
        public ProfileParameters GetParameters(Board board)
        {
            ...
            if ((board.WeaponFriend != null 
                && board.WeaponFriend.Template.Id == Card.Cards.LOOT_542)//手上有武器，而且是弑君
                && board.WeaponFriend.CurrentAtk <2 //手上有武器，武器攻击力小于2
                )
            {
                p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(500)); //弑君不攻击弑君 Kingsbane  ID：LOOT_542
            }
            else
            {
                p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(150)); //弑君不攻击弑君 Kingsbane  ID：LOOT_542
            }
            ...
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_142, new Modifier(-20));//贪婪的书虫 Voracious Reader  ID：SCH_142
            }
            else
            {
            ...
            if ((board.WeaponFriend != null
                && (board.WeaponFriend.Template.Id == Card.Cards.LOOT_542))//手上有武器，而且是弑君
                && board.HasCardInHand(Card.Cards.CS2_074)//致命药膏 Deadly Poison  ID：CS2_074
                || board.HasCardInHand(Card.Cards.GVG_022)//修补匠的磨刀油 Tinker's Sharpsword Oil  ID：GVG_022
                )
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS2_074, new Modifier(-200)); //致命药膏 Deadly Poison  ID：CS2_074
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GVG_022, new Modifier(-200)); //修补匠的磨刀油 Tinker's Sharpsword Oil  ID：GVG_022
            }
            ...
            //手上有弑君和锈水海盗，提高锈水海盗优先值
            if (board.ManaAvailable ==1
              && board.HasCardInHand(Card.Cards.AT_029)//锈水海盗 Buccaneer  ID：AT_029
              && board.HasCardInHand(Card.Cards.LOOT_542)//弑君 Kingsbane  ID：LOOT_542
           )
            {
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(100));//弑君 Kingsbane  ID：LOOT_542
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AT_029, new Modifier(-40));//锈水海盗 Buccaneer  ID：AT_029
            }


            //手上刀是弑君，攻击力大于等于4，提高恐怖海盗 Dread Corsair  ID：NEW1_022
            if ((board.WeaponFriend != null
                && (board.WeaponFriend.Template.Id == Card.Cards.LOOT_542))//手上有武器，而且是弑君
                && board.HasCardInHand(Card.Cards.NEW1_022)//恐怖海盗 Dread Corsair  ID：NEW1_022
                && board.WeaponFriend.CurrentAtk >=4 //手上有武器，武器攻击力小于2
                )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NEW1_022, new Modifier(-80));//恐怖海盗 Dread Corsair  ID：NEW1_022
            }

            //手里有弑君，降低洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
            if (board.HasCardInHand(Card.Cards.LOOT_542)//手上有武器，而且是弑君弑君 Kingsbane  ID：LOOT_542
                && board.HasCardInHand(Card.Cards.NEW1_022)//洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
                )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_033, new Modifier(999));//洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
            }
        

            //手上刀是弑君，攻击力大于等于4，提高劈砍课程 Cutting Class  ID：SCH_623
            if (board.WeaponFriend != null
                && board.HasCardInHand(Card.Cards.SCH_623)//劈砍课程 Cutting Class  ID：SCH_623
                && board.WeaponFriend.CurrentAtk >= 3 //手上有武器，武器攻击力小于2
                )
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_623, new Modifier(-20));//劈砍课程 Cutting Class  ID：SCH_623
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_03bp, new Modifier(999, board.HeroEnemy.Id));
                p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(999)); //弑君不攻击弑君 Kingsbane  ID：LOOT_542
            }

            //有团伙劫掠，弑君，洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033降低英雄优先值
            if (board.HasCardInHand(Card.Cards.LOOT_542)//弑君 Kingsbane  ID：LOOT_542
                || board.HasCardInHand(Card.Cards.LOOT_033)//洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
                || board.HasCardInHand(Card.Cards.TRL_124)//团伙劫掠 Raiding Party  ID：TRL_124
                )
            {
            ...
            //有弑君 Kingsbane  ID：LOOT_542，伺机待发 Preparation  ID：EX1_145，修补匠的磨刀油 Tinker's Sharpsword Oil  ID：GVG_022，提高伺机待发刀油的可能
            if ((board.WeaponFriend != null
                && board.WeaponFriend.Template.Id == Card.Cards.LOOT_542)//手上有武器，而且是弑君
                && board.HasCardInHand(Card.Cards.EX1_145)//伺机待发 Preparation  ID：EX1_145
                && board.HasCardInHand(Card.Cards.GVG_022)//修补匠的磨刀油 Tinker's Sharpsword Oil  ID：GVG_022
                )
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_145, new Modifier(-60));//伺机待发 Preparation  ID：EX1_145
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GVG_022, new Modifier(-60));//修补匠的磨刀油 Tinker's Sharpsword Oil  ID：GVG_022
            }

            //费用大于4且手牌小于3，提高通道优先值
            //if (board.ManaAvailable >= 4
              // && board.WeaponFriend.CurrentAtk < 2 //手上有武器，武器攻击力小于2
               //&& board.HasCardInHand(Card.Cards.SCH_305)//秘密通道 Secret Passage  ID：SCH_305
           //)
            //{
              //  p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_305, new Modifier(-20));//秘密通道 Secret Passage  ID：SCH_305
            //}

            //弑君耐久度大于等于1，不用231
            if ((board.WeaponFriend != null
               && board.WeaponFriend.Template.Id == Card.Cards.LOOT_542)//手上有武器，而且是弑君
               && board.WeaponFriend.CurrentDurability >=1 //手上有武器，武器寿命 = 1
               && board.HasCardInHand(Card.Cards.LOOT_033)//洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
           )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_033, new Modifier(500)); //洞穴探宝者 Cavern Shinyfinder ID：LOOT_033
            }
       else
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_033, new Modifier(-20)); //洞穴探宝者 Cavern Shinyfinder ID：LOOT_033
            }

            //有洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033，不转刀
            if (board.ManaAvailable >= 2
               && board.HasCardInHand(Card.Cards.LOOT_033)//洞穴探宝者 Cavern Shinyfinder  ID：LOOT_033
           )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_033, new Modifier(-20)); //洞穴探宝者 Cavern Shinyfinder ID：LOOT_033
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_03bp, new Modifier(500, board.HeroEnemy.Id));
            }

            //一费不用弑君
            if (board.ManaAvailable ==1
               && board.HasCardInHand(Card.Cards.LOOT_542)//Card.Cards.LOOT_542)//手上有武器，而且是弑君
           )
            {
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(350));//弑君 Kingsbane  ID：LOOT_542
            }
            ...
            if (board.HasCardInHand(Card.Cards.DRG_056)//空降歹徒 Parachute Brigand  ID：DRG_056
                ...
                || board.HasCardInHand(Card.Cards.DRG_035)//血帆飞贼 Bloodsail Flybooter  ID：DRG_035
                || board.HasCardInHand(Card.Cards.DRG_035t)//空中海盗 Sky Pirate  ID：DRG_035t
                || board.HasCardInHand(Card.Cards.NEW1_022)//恐怖海盗 Dread Corsair  ID：NEW1_022
               )
            {
            ...
            //武器优先值
            p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(-40));//弑君 Kingsbane  ID：LOOT_542


            //武器攻击保守性
           // p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.LOOT_542, new Modifier(99)); //弑君不攻击弑君 Kingsbane  ID：LOOT_542
            ...
            //硬币优先级
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CFM_630, new Modifier(40));   //伪造的幸运币 Counterfeit Coin  ID：CFM_630

            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TB_011, new Modifier(40));   //硬币

            //直伤法术优先值		
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS2_075, new Modifier(15));//影袭 Sinister Strike  ID：CS2_075
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_145, new Modifier(20));//伺机待发 Preparation  ID：EX1_145
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AT_035, new Modifier(-40));//危机四伏 Beneath the Grounds  ID：AT_035
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_124, new Modifier(150));//刺骨 Eviscerate  ID：EX1_124
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TRL_124, new Modifier(-10));//团伙劫掠 Raiding Party  ID：TRL_124

            //降低转刀优先值
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.HERO_03bp, new Modifier(250, board.HeroEnemy.Id));

            //随从优先值
            //提高坎雷萨德·埃伯洛克优先级
            //p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BT_309, new Modifier(-80));
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DRG_035, new Modifier(-20));//血帆飞贼 Bloodsail Flybooter  ID：DRG_035
           // p.CastMinionsModifiers.AddOrUpdate(Card.Cards.LOOT_033, new Modifier(-20)); //洞穴探宝者 Cavern Shinyfinder ID：LOOT_033


            //随从优先值

            // p.CastMinionsModifiers.AddOrUpdate(Card.Cards.EX1_613, new Modifier(-20));//艾德温·范克里夫 Edwin VanCleef  ID：EX1_613
            ...
            p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.SCH_248, new Modifier(80)); //修饰甩笔侏儒 Pen Flinger  ID：SCH_248
